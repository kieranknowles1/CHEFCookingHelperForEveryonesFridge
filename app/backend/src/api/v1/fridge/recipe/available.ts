...
export default function installFridgeAvailableRecipeEndpoint (app: Express, db: IChefDatabase): void {
  app.get('/api/v1/fridge/:fridgeId/recipe/available',
    (req: TypedRequest<endpoint>, res: TypedResponse<endpoint, 200>) => {
      ...
      res.json(db.getAvailableRecipes(fridgeId, checkAmounts, maxMissingIngredients))
    })
}
