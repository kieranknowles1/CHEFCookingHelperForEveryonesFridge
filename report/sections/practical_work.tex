...
\ref{sec:why_typescript}, and is styled using Tailwind CSS. Combined, these frameworks allow for a responsive
...
\subsection{Compiler Checks}\label{sec:why_typescript}
\todo{Move to planning}
...
% See figure \ref{fig:type_check} for an example of issues that would not be caught
...
\todo{Sources for coverage and unit testing effectiveness}

Unit tests were chosen as they \todo{source for why they are a good idea}
...
Regression testing was implemented by adding new test cases after fixing an
issue to ensure that the issue was not reintroduced \todo{Source for effectiveness}

As an example, after fixing the round-trip accuracy of storing an embedding in the database
(see section \ref{sec:embedding_round_trip} for an in-depth explanation), test cases were
added that ensure an identical array was returned after encoding it as a buffer
and decoding back in \texttt{testBufferFloat32Array.ts}.
...
For an example, the jscpd linter included with MegaLinter checks for duplicate code blocks. These are indications of code
smell that should be extracted into their own function and reused.~\cite{fowler_refactoring_1997}
