...
One major difference between TypeScript and Java is TypeScript's use of structural typing. This means that any object
that implements the properties specified in an interface is considered an instance of that interface in comparison
to Java where an interface has to be explicitly implemented by a class. As the objects retrieved from the database
did not need their own logic, it was decided early on to do away with having a class for each type in the database
and to instead rely only on interfaces. This simplified the database code and allowed for specialised sub types to easily
be added when needed for different endpoints.
...
it on Windows. \todo{Update this and add benchmark for after hardware accel and fix. Want to run the initial benchmark on my main PC}

\section{Frontend Implementation}
\todo{Frontend}

The front end of the application is written in React with TypeScript for the reasons discussed previously in section
\ref{sec:backend_language}
...
The application is tested using the Mocha test framework \cite{noauthor_mocha_nodate} with API tests additionally using Supertest \cite{noauthor_ladjssupertest_2024}
...
See figure \todo{Figure} for an example of a test report.

\todo{Unit testing}
\todo{Include an example test report}
\todo{Include an example coverage report}
