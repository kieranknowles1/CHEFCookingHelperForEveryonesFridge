...
Due to the analytical nature of the project, user testing was not considered
necessary. Instead, the focus was on ensuring that the system was reliable and the results were as expected. This was done
using automated unit tests for objective areas, i.e., \enquote{did it work?}, and manual testing
for subjective areas such as the quality of the recipe suggestions.

\subsection{Unit Testing}
The backend of the application was tested using the Mocha testing framework,~\cite{noauthor_mocha_nodate} with Supertest~\cite{noauthor_ladjssupertest_2024}
used to simulate API calls. Finally, coverage reports were generated with Istanbul.~\cite{noauthor_istanbul_nodate}
...
for a coverage report, and figure~\ref{fig:test_failure} for an email that was sent if a test failed when a commit
was pushed. All test cases for the backend were contained in the \texttt{app/backend/src/tests} directory. See
figure~\ref{fig:test_case} for an example of a test case.

Test cases were largely written after the functions they were intended to test,
as there was limited evidence for test-driven development leading to higher
quality tests or greater code coverage.~\cite{tosun_effectiveness_2018,madeyski_impact_2010}

Likewise, while reaching a certain level of code coverage was important for detecting
faults, coverage in of itself was not a good indicator of test suite quality.~\cite{meyer_is_2012}
...
The backend is based off the node image, while the frontend uses node during build and nginx at runtime.
Nginx was chosen over Apache as it is significantly more performant than Apache with a smaller memory
when serving static files such as those used to distribute a React app.~\cite{kunda_web_2017}
